 ...
 *
 * `/admin/cases`, `/admin/cases/:id` (A-07/A-08, очередь кейсов) намеренно НЕ зарегистрированы —
 * скрыты полностью по замечанию ПМ (см. PROGRESS.md, «Переработка аналитического модуля»):
 * прямой переход по URL падает на catch-all `*` ниже (404), а не показывает рабочую страницу.
 * Сами файлы (`CasesQueuePage.tsx`, `CaseDetailPage.tsx`, `case.store.ts`, `cases.seed.ts`)
 * не удалены физически — оставлены в коде неиспользуемыми, как было явно указано в задаче.
 */
