...
export function GapByCounterpartyList({ counterparties }: { counterparties: GapCounterparty[] }) {
  const totalGapT = counterparties.reduce((s, c) => s + litersToTonsAvg(c.gapVolumeL), 0);
          ...
          <li key={c.name}>
            <div className="mb-1 flex items-baseline justify-between gap-2 text-xs">
              <span className="flex min-w-0 items-center gap-1.5">
                ...
                {i === 0 && <span className="shrink-0 rounded-full bg-orange-100 px-1.5 py-0.5 text-[10px] font-semibold text-orange-700">крупнейший вклад</span>}
              </span>
              <span className="shrink-0 tabular-nums text-navy-400">
                {litersToTonsAvg(c.gapVolumeL).toLocaleString('ru-RU', { maximumFractionDigits: 0 })} т
                <span className="text-navy-300"> · {c.gapVolumeL.toLocaleString('ru-RU')} л</span> · {c.gapSharePct}%
              </span>
            </div>
